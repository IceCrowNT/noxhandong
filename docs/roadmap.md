...
2. Quản trị nội bộ có login, phân quyền `SUPER_ADMIN` và `MANAGER`.
...
- Manager chỉ xem dữ liệu nội bộ cần thiết, không được import/chốt/xóa dữ liệu nhạy cảm.
...
- Đã chuẩn bị đăng nhập quản trị bằng số điện thoại:
  ...
  - Super Admin tạo manager bắt buộc nhập số điện thoại đăng nhập
...
- Các enum/ký hiệu kỹ thuật như `SUPER_ADMIN`, `MANAGER`, `DA_PUBLIC` vẫn giữ trong schema/DB để bảo toàn logic, nhưng UI hiển thị bằng nhãn tiếng Việt.
