...
async function main() {
  ...
  if (!inputPath) {
    throw new Error(
      "Missing input Excel path. Import the selected workbook from the admin page or pass its path explicitly.",
    );
  }
  ...
  for (const row of dataRows) {
    const paidThroughInfo = parsePaidThroughValue(row.paidThrough);
